...
/**
 * The demo's dam break at several sound speeds and Courant numbers, to find where a scheme goes
        ...
        for (double soundFactor : new double[] {5, 3}) {
            for (double courant : new double[] {0.45, 0.3, 0.2, 0.1}) {
                ...
                try (Rig rig = Rig.on(Backend.GPU, step)) {
                    ...
                    System.out.printf("[sweep] c=%.0fx C=%.2f: %5d steps, y95 %.1f, J %.2f..%.2f, "
                                    + "fastest %.2f of fall speed%n", soundFactor, courant, steps,
