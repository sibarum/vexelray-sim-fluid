 ...
 * number, the cell size and the dry threshold are data, read from the {@link #PARAMS} buffer, and the time the
 * step may spend from {@link #BUDGET_IN}, so tuning them never rebuilds the kernel
 * ({@code docs/architecture.md}, <i>structure is compiled, values are data</i>). Buffers rather than push
 * constants because those carry the dispatch's invocation count, and the CPU backend has none.
 *
 ...
 * was last read by the previous step, which is finished, and is next written by the following one.
 *
 * <h2>The kernel spends time; it does not keep it</h2>
 * The kernel never needs to know what time it is — only how much it may still spend. So the absolute clock is
 * the host's ({@link Clock}), an i64 on the CPU where 64-bit integers are free, and the kernel is handed a
 * <em>budget</em>: at most this many ticks. Each step is the smaller of what the fastest wave allows and what is
 * left, rounded to whole ticks <em>first</em> so the physics steps by exactly the time spent, and invocation zero
 * writes back the remainder. Once the budget is spent, steps move nothing, so a caller can overshoot the step
 * count freely and read the remainder to know how far it got. The budget ping-pongs with the state.
 *
 * <p>Time is an integer throughout: a {@link #TICKS_PER_SECOND microsecond} count, a rational with a fixed
 * denominator. A float clock built by adding a step each step rounds each addition, and with a near-constant
 * step the roundings share a sign — within the hour an f32 is off by a few percent of a step per step, and even
 * an f64 could drift by a day a century. Integer additions do not round, so nothing drifts. And because the GPU
 * holds only a delta, everything on it fits an i32: a budget or a single step of up to ~35 minutes, no 64-bit
 * integers, so no optional device capability the kernel would otherwise fall back to the CPU without.
 */
public final class ShallowWater {
    ...
    /** One element: the ticks this step may spend. */
     ...
     * is unbounded, and the cap keeps its tick count inside an i32: 2000 s is 2·10⁹ of the 2.147·10⁹ there are.
     */
    private static final double LONGEST_STEP = 2000;
    ...
    public static Function kernel(int nx, int ny, Edges edges) {
        ...
        // The step: as large as the fastest wave allows, in whole ticks, and no more than the budget holds. The
        // ticks are decided first and the physics steps by them, so the time spent is the time the water moved.
        ...
        LocalVar allowed = b.let("allowed", min(div(mul(v(courant), min(v(dx), v(dy))), max(v(fastest), f(1e-30))),
                f(LONGEST_STEP)));
        LocalVar ticks = b.let("ticks", toInt(mul(v(allowed), f(TICKS_PER_SECOND))));
        LocalVar budget = b.let("budget", load(BUDGET_IN, i(0)));
        b.when(lt(v(budget), v(ticks)), t -> t.set(ticks, v(budget)));
        ...
        b.when(eq(v(cell), i(0)), t -> {
            ...
            t.store(BUDGET_OUT, i(0), sub(v(budget), v(ticks)));
        });
    ...
    /** {@code seconds} as ticks, to the nearest microsecond. */
